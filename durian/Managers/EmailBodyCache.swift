...
class EmailBodyCache: ObservableObject {
    ...
    private init() {
        ...
        let cacheDir = appSupport.appendingPathComponent("durian", isDirectory: true)
    ...
    private func migrateFromJSON() {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let oldCacheDir = appSupport.appendingPathComponent("durian/EmailBodyCache", isDirectory: true)
