...
func TestDangerousElementsRemoved(t *testing.T) {
	tests := []struct {
		name  string
		input string
		...
		{"style tag", `<style>body{display:none}</style><p>text</p>`, "style"},
		{"iframe", `<iframe src="http://evil.com"></iframe>`, "iframe"},
		{"form", `<form action="/steal"><input type="text"></form>`, "form"},
		{"object", `<object data="evil.swf"></object>`, "object"},
		{"embed", `<embed src="evil.swf">`, "embed"},
		{"svg", `<svg onload="alert(1)"><circle r="10"/></svg>`, "svg"},
		...
		{"textarea", `<textarea>hidden</textarea>`, "textarea"},
		{"button", `<button onclick="evil()">Click</button>`, "button"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := SanitizeHTML(tt.input)
...
func TestCSSPropertyWhitelist(t *testing.T) {
	tests := []struct {
		...
		{"display blocked", `<p style="display: none">hidden</p>`, false},
		{"position blocked", `<p style="position: absolute">text</p>`, false},
		{"width blocked", `<p style="width: 9999px">text</p>`, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := SanitizeHTML(tt.input)
...
func TestCSSExpressionBlocked(t *testing.T) {
	tests := []string{
		...
		`<p style="background-color: url(javascript:alert(1))">text</p>`,
	}
	for _, input := range tests {
		got := SanitizeHTML(input)
		if strings.Contains(got, "style=") {
			t.Errorf("dangerous CSS value not blocked in %q, got %q", input, got)
		}
	}
}
...
func TestFontTagStripped(t *testing.T) {
	got := SanitizeHTML(`<font color="red">text</font>`)
	if strings.Contains(got, "<font") {
		t.Errorf("font tag should be stripped, got %q", got)
	}
