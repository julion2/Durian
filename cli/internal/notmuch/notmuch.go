...
func (c *Client) GetAllMessagesWithTags(folder string) (map[string][]string, error) {
	...
	// Parse the nested JSON structure from notmuch show.
	// The format is deeply recursive: [[{msg}, [replies]], ...]
	// where replies have the same structure, nested arbitrarily deep.
	// We walk the entire tree to find all message objects.
...
// message id/tags from every message object it finds, regardless of nesting depth.
func extractMessages(raw json.RawMessage, result map[string][]string) {
	// Try to decode as a message object (has "id" field)
	var msg struct {
		ID   string   `json:"id"`
		Tags []string `json:"tags"`
	}
	...
	// Otherwise try as an array and recurse into each element
